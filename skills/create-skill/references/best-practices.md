...
**Rules:**
- Routing and decision tables belong in SKILL.md only — sub-files should never route back to siblings
- Shared config goes in SKILL.md once; mode-specific config goes in each sub-file once
...
### Avoid "optionally" when conditions are defined
...
### Avoid hardcoding specific MCP server names

When a skill needs a *category* of tool (e.g., documentation lookup), reference the category generically. Different projects have different MCP servers installed.
